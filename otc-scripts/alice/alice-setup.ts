...
// Using the same scheme ID configured for your setup
const currentSchemeIdForAlice = config.ANCILE_SCHEME_ID_FOR_ACCOUNT_SETUP;

async function aliceBackendSetup() {
    console.log("⚙️  Initializing Alice's Privacy Setup (OTC Workflow)...");
    ...
    console.log(`👤 Alice's Main Address:  ${account.address}`);
    ...
    const signature = await walletClient.signTypedData({
        ...
        message: {
            schemeId: BigInt(currentSchemeIdForAlice),
    ...
    if (!fs.existsSync(proofPath)) {
        throw new Error(`❌ Missing world-id-proof.json! Alice must complete World ID verification first. Expected: ${addressProofPath}`);
    }
    ...
    // Final Payload
    const newPayload = {
        action: 1, // ActionType.REGISTER
        data: {
            registrant: account.address,
            schemeId: currentSchemeIdForAlice,
    ...
    console.log(`✅ Saved Payload to alice-latest-payload.json ready for CRE dispatch!`);
}

aliceBackendSetup().catch(console.error);

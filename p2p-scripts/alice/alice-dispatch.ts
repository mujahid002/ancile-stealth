...
async function runAliceDispatch() {
    ...
    const newPayload = {
        action: 2,
        data: {
            token: config.TOKEN_ADDRESS, amount: amountToProcess.toString(), sender: account.address, recipientRegistrant: config.BOB_PUBLIC_ADDRESS,

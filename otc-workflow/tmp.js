...
var bytesToHex4 = (b) => Array.from(abytes2(b)).map((e) => padh(e, 2)).join("");
...
var toU8 = (a, len2) => abytes2(isStr(a) ? hexToBytes4(a) : u8fr(abytes2(a)), len2);
...
Point.BASE = G;
Point.ZERO = I;
var bytesToNumBE = (b) => big("0x" + (bytesToHex4(b) || "0"));
var sliceBytesNumBE = (b, from2, to) => bytesToNumBE(b.subarray(from2, to));
var B256 = 2n ** 256n;
var numTo32b = (num) => hexToBytes4(padh(arange(num, 0n, B256), L2));
var toPrivScalar = (pr) => {
...
var randomPrivateKey = () => hashToPrivateKey(randomBytes2(L + 16));
var utils2 = {
...
var VALID_SCHEME_ID;
(function(VALID_SCHEME_ID2) {
  VALID_SCHEME_ID2[VALID_SCHEME_ID2["SCHEME_ID_1"] = 1] = "SCHEME_ID_1";
})(VALID_SCHEME_ID || (VALID_SCHEME_ID = {}));
function generateStealthAddress({ stealthMetaAddressURI, schemeId = VALID_SCHEME_ID.SCHEME_ID_1, ephemeralPrivateKey }) {
  ...
  const ephemeralPublicKey = getPublicKey2({
  ...
  const stealthAddress = publicKeyToAddress2({
...
function validateStealthMetaAddress({ stealthMetaAddress, schemeId }) {
  handleSchemeId(schemeId);
  const cleanedStealthMetaAddress = stealthMetaAddress.startsWith("0x") ? stealthMetaAddress.substring(2) : stealthMetaAddress;
  ...
  const singlePublicKeyHexLength = 66;
  const spendingPublicKey = cleanedStealthMetaAddress.slice(0, singlePublicKeyHexLength);
  const viewingPublicKey = cleanedStealthMetaAddress.length === 132 ? cleanedStealthMetaAddress.slice(singlePublicKeyHexLength) : spendingPublicKey;
...
function isValidCompressedPublicKey(publicKey) {
  return (publicKey.startsWith("02") || publicKey.startsWith("03")) && publicKey.length === 66;
}
function parseKeysFromStealthMetaAddress({ stealthMetaAddress, schemeId }) {
  ...
  const singlePublicKeyLength = 66;
  const spendingPublicKey = cleanedStealthMetaAddress.slice(0, singlePublicKeyLength);
  const viewingPublicKey = cleanedStealthMetaAddress.length === 132 ? cleanedStealthMetaAddress.slice(singlePublicKeyLength) : spendingPublicKey;
  return {
...
function generatePrivateKey({ ephemeralPrivateKey, schemeId }) {
  handleSchemeId(schemeId);
  if (ephemeralPrivateKey && utils2.isValidPrivateKey(ephemeralPrivateKey)) {
    return ephemeralPrivateKey;
  }
  return utils2.randomPrivateKey();
}
function getPublicKey2({ privateKey, compressed, schemeId }) {
  handleSchemeId(schemeId);
...
function getStealthPublicKey({ spendingPublicKey, hashedSharedSecret, schemeId }) {
  handleSchemeId(schemeId);
  const hashedSharedSecretPoint = Point.fromPrivateKey(hexToBytes2(hashedSharedSecret));
  return Point.fromHex(spendingPublicKey).add(hashedSharedSecretPoint).toRawBytes(false);
}
function publicKeyToAddress2({ publicKey, schemeId }) {
  handleSchemeId(schemeId);
  return publicKeyToAddress(typeof publicKey !== "string" ? bytesToHex2(publicKey) : publicKey);
}
var ERC5564_StartBlocks;
(function(ERC5564_StartBlocks2) {
  ERC5564_StartBlocks2[ERC5564_StartBlocks2["ARBITRUM_ONE"] = 219468264] = "ARBITRUM_ONE";
  ERC5564_StartBlocks2[ERC5564_StartBlocks2["ARBITRUM_SEPOLIA"] = 24849998] = "ARBITRUM_SEPOLIA";
  ERC5564_StartBlocks2[ERC5564_StartBlocks2["BASE"] = 15502414] = "BASE";
  ERC5564_StartBlocks2[ERC5564_StartBlocks2["BASE_SEPOLIA"] = 7552655] = "BASE_SEPOLIA";
  ERC5564_StartBlocks2[ERC5564_StartBlocks2["HOLESKY"] = 1222405] = "HOLESKY";
  ERC5564_StartBlocks2[ERC5564_StartBlocks2["MAINNET"] = 20042207] = "MAINNET";
  ERC5564_StartBlocks2[ERC5564_StartBlocks2["MATIC"] = 57888814] = "MATIC";
  ERC5564_StartBlocks2[ERC5564_StartBlocks2["OPTIMISM"] = 121097390] = "OPTIMISM";
  ERC5564_StartBlocks2[ERC5564_StartBlocks2["OPTIMISM_SEPOLIA"] = 9534458] = "OPTIMISM_SEPOLIA";
  ERC5564_StartBlocks2[ERC5564_StartBlocks2["SEPOLIA"] = 5486597] = "SEPOLIA";
})(ERC5564_StartBlocks || (ERC5564_StartBlocks = {}));
var ERC6538_StartBlocks;
(function(ERC6538_StartBlocks2) {
  ERC6538_StartBlocks2[ERC6538_StartBlocks2["ARBITRUM_ONE"] = 219468267] = "ARBITRUM_ONE";
  ERC6538_StartBlocks2[ERC6538_StartBlocks2["ARBITRUM_SEPOLIA"] = 25796636] = "ARBITRUM_SEPOLIA";
  ERC6538_StartBlocks2[ERC6538_StartBlocks2["BASE"] = 15502414] = "BASE";
  ERC6538_StartBlocks2[ERC6538_StartBlocks2["BASE_SEPOLIA"] = 7675097] = "BASE_SEPOLIA";
  ERC6538_StartBlocks2[ERC6538_StartBlocks2["HOLESKY"] = 1222405] = "HOLESKY";
  ERC6538_StartBlocks2[ERC6538_StartBlocks2["MAINNET"] = 20042207] = "MAINNET";
  ERC6538_StartBlocks2[ERC6538_StartBlocks2["MATIC"] = 57888814] = "MATIC";
  ERC6538_StartBlocks2[ERC6538_StartBlocks2["OPTIMISM"] = 121097390] = "OPTIMISM";
  ERC6538_StartBlocks2[ERC6538_StartBlocks2["OPTIMISM_SEPOLIA"] = 9658043] = "OPTIMISM_SEPOLIA";
  ERC6538_StartBlocks2[ERC6538_StartBlocks2["SEPOLIA"] = 5538412] = "SEPOLIA";
})(ERC6538_StartBlocks || (ERC6538_StartBlocks = {}));
var contracts = {
...
var PRIMARY_TYPE = "Erc6538RegistryEntry";
var SIGNATURE_TYPES = {
  [PRIMARY_TYPE]: [
...
var configSchema = exports_external.object({
  evms: exports_external.array(exports_external.object({ receiverAddress: exports_external.string(), chainSelectorName: exports_external.string(), gasLimit: exports_external.string(), isTestnet: exports_external.boolean().optional() }))
});
var onOtcRoute = async (runtime2, payload) => {
  runtime2.log("\uD83E\uDD1D Initializing Ancile OTC Matchmaker...");
  ...
  const alicePayload = payloads.find((p) => p.maker === "Alice");
  const bobPayload = payloads.find((p) => p.maker === "Bob");
  if (!alicePayload || !bobPayload || !entropyA || !entropyB) {
    throw new Error("❌ Missing payloads or entropy for match");
  }
  runtime2.log("\uD83E\uDDEE Generating Double-Blind Stealth Addresses...");
  const stealthA = generateStealthAddress({
    stealthMetaAddressURI: `st:eth:${alicePayload.stealthMetaAddress}`,
    ephemeralPrivateKey: hexToBytes2(entropyA)
  });
  const stealthB = generateStealthAddress({
    stealthMetaAddressURI: `st:eth:${bobPayload.stealthMetaAddress}`,
    ephemeralPrivateKey: hexToBytes2(entropyB)
  });
  const sigA = parseSignature(alicePayload.permit.signature);
  const sigB = parseSignature(bobPayload.permit.signature);
  const actionType = 5;
  runtime2.log("\uD83D\uDCE6 Bundling Dual-Permit Payload...");
  const otcAbi = [{
    type: "tuple",
    components: [
      ...
      { name: "deadlineA", type: "uint256" },
      { name: "vA", type: "uint8" },
      { name: "rA", type: "bytes32" },
      { name: "sA", type: "bytes32" },
      { name: "stealthAddressB", type: "address" },
      { name: "ephemeralPubKeyB", type: "bytes" },
      { name: "tokenB", type: "address" },
      { name: "ownerB", type: "address" },
      { name: "amountB", type: "uint256" },
      { name: "deadlineB", type: "uint256" },
      { name: "vB", type: "uint8" },
      { name: "rB", type: "bytes32" },
      { name: "sB", type: "bytes32" },
      { name: "stealthAddressA", type: "address" },
      { name: "ephemeralPubKeyA", type: "bytes" }
    ]
  }];
  const nestedPayloadBytes = encodeAbiParameters(otcAbi, [{
    tokenA: alicePayload.giveToken,
    ownerA: alicePayload.permit.owner,
    amountA: BigInt(alicePayload.giveAmount),
    deadlineA: BigInt(alicePayload.permit.deadline),
    vA: Number(sigA.v),
    rA: sigA.r,
    sA: sigA.s,
    stealthAddressB: stealthB.stealthAddress,
    ephemeralPubKeyB: stealthB.ephemeralPublicKey,
    tokenB: bobPayload.giveToken,
    ownerB: bobPayload.permit.owner,
    amountB: BigInt(bobPayload.giveAmount),
    deadlineB: BigInt(bobPayload.permit.deadline),
    vB: Number(sigB.v),
    rB: sigB.r,
    sB: sigB.s,
    stealthAddressA: stealthA.stealthAddress,
    ephemeralPubKeyA: stealthA.ephemeralPublicKey
  }]);
  ...
  runtime2.log(`\uD83D\uDE80 Dispatching OTC Settlement to Router: ${evmConfig.receiverAddress}`);
  ...
  runtime2.log(`✅ OTC Settlement Complete! Tx Hash: ${txHash}`);
  return `OTC Execution complete. Tx Hash: ${txHash}`;
};
async function main() {
  const runner = await Runner.newRunner({ configSchema });
  await runner.run((config2) => [handler(new HTTPCapability().trigger({}), onOtcRoute)]);
}

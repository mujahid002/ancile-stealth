...
var configSchema = exports_external.object({
  evms: exports_external.array(exports_external.object({
    ...
    isTestnet: exports_external.boolean().optional()
  }))
});
var onAncileRoute = async (runtime2, payload) => {
  ...
  let logPrefix;
  if (isSweep) {
    runtime2.log("\uD83E\uDDF9 Initializing Ancile Gasless Sweep Relayer...");
    logPrefix = "Batch Sweep";
    const formattedSweeps = payloads.map((sweep) => {
      const pSig = parseSignature(sweep.permit.signature);
      const iSig = parseSignature(sweep.intent.signature);
      return {
        token: sweep.token,
        amount: BigInt(sweep.amount),
        stealthAddress: sweep.stealthAddress,
        destination: sweep.destination,
        permit: { deadline: BigInt(sweep.permit.deadline), v: Number(pSig.v), r: pSig.r, s: pSig.s },
        intent: { v: Number(iSig.v), r: iSig.r, s: iSig.s }
      };
    });
    const sweepAbi = [{
      type: "tuple[]",
    ...
    if (!alicePayload || !bobPayload)
      throw new Error("❌ Missing payloads for OTC match");
    runtime2.log("\uD83E\uDDEE Parsing Client-Side Shards...");
    const sigA = parseSignature(alicePayload.permit.signature);
    const sigB = parseSignature(bobPayload.permit.signature);
    const pulls = [
      { token: alicePayload.giveToken, owner: alicePayload.permit.owner, amount: BigInt(alicePayload.giveAmount), deadline: BigInt(alicePayload.permit.deadline), v: Number(sigA.v), r: sigA.r, s: sigA.s },
      { token: bobPayload.giveToken, owner: bobPayload.permit.owner, amount: BigInt(bobPayload.giveAmount), deadline: BigInt(bobPayload.permit.deadline), v: Number(sigB.v), r: sigB.r, s: sigB.s }
    ];
    const pushes = [];
    ...
    for (const shard of bobPayload.receivingShards) {
      pushes.push({ token: alicePayload.giveToken, to: shard, amount: bobChunk });
    }
    ...
    for (const shard of alicePayload.receivingShards) {
      pushes.push({ token: bobPayload.giveToken, to: shard, amount: aliceChunk });
    }
    runtime2.log(`\uD83D\uDCE6 Bundling ${pulls.length} Pulls and ${pushes.length} Pushes...`);
    const batchAbi = [
  ...
  runtime2.log(`\uD83D\uDE80 Dispatching ${logPrefix} to Router: ${evmConfig.receiverAddress}`);
  ...
  runtime2.log(`✅ ${logPrefix} Settlement Complete! Tx Hash: ${txHash}`);
